...
void Foliage( mapDrawSurface_t *src )
{
	...
	for( foliage = si->foliage; foliage != NULL; foliage = foliage->next )
	{
		...
		InsertModel( foliage->model, 0, transform, 1.0, NULL, NULL, src->entityNum, src->castShadows, src->recvShadows, 0, src->lightmapScale, 0, src->smoothNormals, src->vertTexProj );
